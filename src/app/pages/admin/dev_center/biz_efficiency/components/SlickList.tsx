...
import { BusinessReportData } from "../data";
...
const SlickList = () => {
  ...
  const [value, setValue] = useState<number>(BusinessReportData(t)[0].id);
  ...
  useEffect(() => {
    ...
    if (carouselItem && carouselItem instanceof HTMLElement && value !== null) {
      const selectedItem = BusinessReportData(t).find((item) => item.id === value);
      if (selectedItem) {
